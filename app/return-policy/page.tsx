...
export const metadata = {
  title: 'Return Policy',
  alternates: { canonical: 'https://shop.prag.global/return-policy' },
};

export default async function ReturnPolicyPage() {
  const [content, wpPage] = await Promise.all([
    getB2CPublicContent(),
    getPage('return-policy'),
  ]);
  const page = findB2CPage(content, '/return-policy');
  const heroSection = findVisibleSectionsByType(page, 'hero')[0];
  const heroTitle = heroSection?.summary || 'Return Policy';
