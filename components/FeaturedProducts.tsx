  ...
  return (
    <div className="w-full px-4 md:px-20 py-10 md:py-14 bg-stone-50 flex flex-col justify-center items-center gap-8">
      <div className="w-full max-w-[1280px] flex flex-col justify-center items-center gap-8 md:gap-12">
        <h2 className="w-full max-w-[631px] text-center text-base md:text-2xl font-bold font-['Montserrat'] text-black">
          Featured Products
        </h2>

        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-5 gap-y-9 md:gap-x-6 md:gap-y-11">
          {products.slice(0, 8).map((product, idx) => {
            const isNew = product.tags?.some((tag) => tag.slug === 'new' || tag.name.toLowerCase().includes('new')) ?? false;
            return (
              <div key={product.id} className={idx >= 4 ? 'hidden md:block' : ''}>
                <ProductCard product={product} isNew={isNew} priority={idx === 0} priceColor={LISTING_PRICE_COLOR} />
              </div>
            );
          })}
        </div>
