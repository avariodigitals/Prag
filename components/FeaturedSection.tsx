...
function FeaturedCard({ product, whatsappDigits }: { product: Product; whatsappDigits: string }) {
...
export default function FeaturedSection({ products, whatsappNumber, settings }: FeaturedSectionProps) {
  if (products.length === 0) return null;

  const enabled = settings?.featured_section_enabled ?? true;
      ...
      <div className="w-full max-w-[1280px] flex flex-col justify-center items-center gap-10 md:gap-14">
        ...
        <div className="w-full flex flex-col items-center gap-3 md:gap-4 text-center">
          ...
          <p className="text-zinc-500 text-sm md:text-base font-normal font-['Montserrat']">
            Hand-picked power solutions our customers love.
          </p>
        </div>
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-5 gap-y-8 md:gap-x-6 md:gap-y-10">
          {visible.map((product) => (
            <FeaturedCard key={product.id} product={product} whatsappDigits={whatsappDigits} />
          ))}
        </div>
