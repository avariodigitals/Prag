...
const ASSURANCE_FEATURES = [
  { icon: ShieldCheck, title: 'Genuine products', description: 'Configured for Nigerian conditions.' },
  { icon: Truck, title: 'Nationwide delivery', description: 'Fast fulfilment from Lagos.' },
  { icon: LifeBuoy, title: 'After-sales support', description: 'Setup guidance and maintenance.' },
];

function AssuranceCard() {
  return (
    <div className="rounded-2xl border border-zinc-200/80 bg-gradient-to-br from-sky-50/60 to-white p-4 md:p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        <ShieldCheck className="w-4 h-4 text-sky-700" aria-hidden="true" />
        <span className="text-[11px] font-semibold font-['Montserrat'] uppercase tracking-[0.14em] text-sky-700">
          Why shop with PRAG
        </span>
      </div>
      <div className="grid grid-cols-3 divide-x divide-zinc-200/80">
        {ASSURANCE_FEATURES.map((feature) => {
          const Icon = feature.icon;
          return (
            <div key={feature.title} className="flex flex-col items-center text-center gap-1.5 px-2 first:pl-0 last:pr-0">
              <div className="w-9 h-9 rounded-xl bg-white border border-sky-100 flex items-center justify-center shrink-0 shadow-sm">
                <Icon className="w-[18px] h-[18px] text-sky-700" aria-hidden="true" />
              </div>
              <span className="text-zinc-900 text-[12px] font-semibold font-['Montserrat'] leading-tight">{feature.title}</span>
              <span className="text-zinc-500 text-[10px] font-normal font-['Montserrat'] leading-snug">{feature.description}</span>
            </div>
          );
        })}
      </div>
...
function ShowroomsCard({ offices }: { offices: Store[] }) {
  return (
    <div className="rounded-2xl border border-zinc-200/80 bg-white p-4 md:p-5 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
      ...
      {offices.length > 0 ? (
        <div className="flex flex-col gap-2">
          {offices.map((office) => (
            <div key={office.id} className="flex items-center justify-between gap-3 rounded-xl bg-zinc-50/70 px-3 py-2 hover:bg-sky-50/70 transition-colors">
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="w-1.5 h-1.5 rounded-full bg-sky-600 shrink-0" />
                <span className="text-zinc-900 text-[13px] font-medium font-['Montserrat'] truncate">{office.name}</span>
              </div>
              {office.map_url ? (
                <a href={office.map_url} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-sky-700 text-[11px] font-semibold font-['Montserrat'] whitespace-nowrap hover:text-sky-900 transition-colors">
                  Map
        ...
        <Link href="/stores"
          className="inline-flex items-center gap-1 text-sky-700 text-[13px] font-medium font-['Montserrat'] hover:text-sky-900 transition-colors">
          Lagos &amp; Abuja offices
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start md:items-start">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-5 md:gap-6 max-w-[620px]">
          ...
          <div className="flex gap-4">
            <a href={`https://wa.me/2348032170129?text=${encodeURIComponent(`Hi, I'm interested in the ${product.name}. ${pageUrl}`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 h-11 px-4 bg-sky-700 rounded-3xl flex justify-center items-center gap-2 hover:bg-sky-800 transition-colors">
              ...
              <span className="text-white text-sm font-medium font-['Montserrat']">Order on WhatsApp</span>
            </a>
          ...
          <button onClick={handleAddToCart} disabled={adding || addedToCart || isOutOfStock}
            className="w-full h-11 px-4 bg-white rounded-3xl outline outline-1 outline-sky-700 flex justify-center items-center gap-2 hover:bg-sky-50 transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
            {adding ? (
              <svg className="w-4 h-4 text-sky-700 animate-spin" fill="none" viewBox="0 0 24 24">
              ...
              <>
                ...
                <span className="text-sky-700 text-sm font-medium font-['Montserrat']">Added to Cart</span>
              </>
            ) : (
              <span className="text-sky-700 text-sm font-medium font-['Montserrat']">Add to Cart</span>
            )}
          </button>
        ...
        {activeTab === 'Specifications' && (
          <div className="flex flex-col gap-4">
            ...
            {customTabs.length > 0 ? (
              ...
              <p className="text-zinc-400 font-['Montserrat']">No specifications available for this product.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'Technical Resources' && (
          <div className="p-2 md:p-3 bg-white flex flex-col gap-3 md:gap-4">
            {techDocs.length > 0 ? (
              ...
              <p className="text-zinc-400 font-['Montserrat']">No technical documents available for this product.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'Reviews' && (
          <div className="flex flex-col gap-8">
                ...
                return (
                  <div key={review.id} className="flex flex-col gap-2.5 pb-7 border-b border-gray-100 last:border-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-zinc-500 text-base md:text-lg leading-none font-normal font-['Montserrat']">{new Date(review.date_created).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </div>
                    {chunks.title && (
                      <h4 className="text-zinc-900 text-xl md:text-2xl font-semibold leading-tight font-['Montserrat']">{chunks.title}</h4>
                    )}
                    <p className="text-zinc-700 text-base md:text-lg leading-relaxed font-normal font-['Montserrat']">{chunks.body || chunks.title}</p>
                  </div>
                );
              })
            ) : (
              <p className="text-zinc-400 font-['Montserrat']">No reviews yet for this product.</p>
            )}
            ...
            <form onSubmit={handleSubmitReview} className="flex flex-col gap-4 p-4 md:p-5 bg-stone-50 rounded-2xl">
              ...
              {reviewNotice && (
                <p className="text-base md:text-lg font-['Montserrat'] text-sky-700">{reviewNotice}</p>
              )}
            </form>
          </div>
        )}
