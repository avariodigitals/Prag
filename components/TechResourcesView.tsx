    ...
    <div className="w-full px-4 md:px-20 py-10 md:py-24 flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <span className="text-zinc-900 text-base font-normal font-['Montserrat']">SELECT PRODUCT</span>
        </div>
      ...
      {!selectedProduct && (
        <p className="text-gray-400 text-base md:text-lg font-['Montserrat'] text-center py-10">
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-10">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
        <h2 className="text-zinc-900 text-2xl md:text-3xl font-bold font-['Montserrat'] leading-tight">{product.name}</h2>
        {!loading && <span className="text-zinc-500 text-base md:text-lg font-normal font-['Montserrat'] leading-6">{docs.length} document{docs.length !== 1 ? 's' : ''} available</span>}
      </div>
      ...
      {!loading && docs.length === 0 && (
        <p className="text-gray-400 text-base font-['Montserrat'] py-6">No documents available for this product.</p>
      )}
            ...
            <div key={doc.id} className="p-4 md:p-6 bg-white rounded-2xl outline outline-1 outline-neutral-200 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
              <div className="flex items-center gap-4 md:gap-6">
                ...
                <div className="flex flex-col gap-1 md:gap-3">
                  <span className="text-zinc-900 text-lg md:text-2xl font-medium font-['Montserrat'] leading-7">{doc.title}</span>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {doc.file_type && <span className="text-zinc-500 text-base md:text-lg font-normal font-['Montserrat']">{doc.file_type.toUpperCase()}</span>}
                    {doc.file_size && <><div className="w-1 h-1 bg-sky-700 rounded-full" /><span className="text-zinc-500 text-base md:text-lg font-normal font-['Montserrat']">{doc.file_size}</span></>}
                    {doc.pages && <><div className="w-1 h-1 bg-sky-700 rounded-full" /><span className="text-zinc-500 text-base md:text-lg font-normal font-['Montserrat']">{doc.pages} pages</span></>}
                  </div>
                </div>
              </div>
              <a href={doc.file_url} target="_blank" rel="noopener noreferrer" download
                ...
                <span className="text-white text-base font-medium font-['Montserrat'] leading-6">Download</span>
              </a>
            </div>
          ))}
        </div>
