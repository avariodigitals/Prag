...
function SearchBox({ mobile = false }: { mobile?: boolean }) {
  ...
  return (
    <div ref={containerRef} className={mobile ? 'flex-1 relative' : 'hidden lg:block relative flex-1 max-w-[566px]'}>
    ...
    <>
      ...
      <div className="hidden lg:flex w-full px-4 xl:px-10 2xl:px-20 py-3 bg-white items-center gap-3 xl:gap-4 flex-wrap">
        <Link href="/" aria-label="PRAG home" className="shrink-0">
          <Image src="/Prag Logo.png" alt="PRAG" width={112} height={32} priority />
        </Link>
        ...
        <div className="ml-auto inline-flex items-center gap-2 xl:gap-4 shrink min-w-0 flex-wrap justify-end">
          <a href={`https://wa.me/${whatsapp.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="py-1.5 flex items-center gap-1.5 min-w-0">
            ...
            <div className="flex flex-col leading-tight min-w-0">
              <span className="text-neutral-700 text-xs xl:text-sm font-bold font-['Space_Grotesk']">Hotline</span>
              <span className="text-neutral-700 text-xs xl:text-sm font-medium font-['Space_Grotesk'] whitespace-nowrap">{phone}</span>
            </div>
          </a>
          ...
          {user ? (
            ...
            <Link href="/login" className="p-2.5 flex items-center gap-2 hover:bg-stone-50 rounded-lg transition-colors min-w-0">
              ...
              <span className="text-neutral-700/70 text-sm xl:text-base font-medium font-['Space_Grotesk'] leading-4 whitespace-nowrap">Login or Register</span>
            </Link>
          )}
        </div>
      ...
      <div className="lg:hidden sticky top-0 z-50 w-full bg-white/95 backdrop-blur-sm shadow-[0_1px_0_rgba(15,23,42,0.08)]">
        <div className="w-full px-4 h-[72px] bg-white flex items-center justify-between gap-4">
          <Link href="/" aria-label="PRAG home" className="shrink-0">
            <Image src="/Prag Logo.png" alt="PRAG" width={100} height={32} priority className="h-7 w-auto" />
          </Link>

          <div className="flex items-center gap-2 shrink-0">
            <Link href="/wishlist" aria-label="Wishlist" className="w-12 h-12 flex items-center justify-center rounded-full active:bg-black/5 transition-colors">
        ...
        <div className="w-full px-4 py-3 bg-sky-700 flex items-center gap-3">
          <button onClick={() => setMenuOpen(true)} aria-label="Open menu" className="shrink-0 w-12 h-12 flex items-center justify-center rounded-lg active:bg-white/10 transition-colors">
            <Menu className="w-8 h-8 text-white" strokeWidth={2.1} />
          </button>
          <SearchBox mobile />
        </div>
      </div>
