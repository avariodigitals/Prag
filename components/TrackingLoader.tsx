...
const WHATSAPP_QUICK_ACTIONS = [
  { label: 'General Enquiries', message: 'Hi, I have a general enquiry about your products.' },
  { label: 'Delivery', message: 'Hi, I need to inquire about delivery options.' },
  { label: 'Sales', message: 'Hi, I am interested in purchasing from PRAG.' },
  { label: 'Support', message: 'Hello, I need support from PRAG.' },
];
  ...
  return (
    <>
      ...
      {isWhatsappEnabled && hasWhatsappNumber && (
        <div className="fixed bottom-24 right-4 z-[80] flex flex-col items-end gap-2 md:bottom-24 md:right-6">
          {isMenuOpen && (
            <div className="w-[220px] rounded-xl border border-[#D8F3E1] bg-white p-2 shadow-xl">
              <p className="px-2 pb-1 text-xs font-semibold text-gray-500">{whatsappText}</p>
              <div className="flex flex-col gap-1">
                {WHATSAPP_QUICK_ACTIONS.map((action) => (
                  <button
                    key={action.label}
                    type="button"
                    className="rounded-lg px-3 py-2 text-left text-sm font-medium text-gray-700 transition-colors hover:bg-[#E9FBEF]"
          ...
          <button
            ...
            className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg transition-transform duration-200 hover:scale-[1.06] hover:bg-[#1ebe5d] md:h-16 md:w-16"
          >
